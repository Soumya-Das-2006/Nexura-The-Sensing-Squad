"""
apps/fraud/fraud_service.py

Single-responsibility fraud scoring service.
Loads IsolationForest + XGBoost once per process.
Exposes one public function: score_claim(claim) -> float (0.0 = clean, 1.0 = fraud)
"""
import math
import logging
from pathlib import Path
from django.conf import settings

logger = logging.getLogger(__name__)

# ── Module-level cache ────────────────────────────────────────────────────────
_iso_forest   = None
_xgb_model    = None
_feature_cols = None
_iso_s_min    = None
_iso_s_max    = None
_loaded       = False


def _load() -> bool:
    """
    Lazy-load fraud models from settings.ML_MODELS_DIR.
    Idempotent — safe to call multiple times.
    Returns True only if BOTH models load successfully.
    """
    global _iso_forest, _xgb_model, _feature_cols
    global _iso_s_min, _iso_s_max, _loaded

    if _loaded:
        return True

    models_dir: Path = settings.ML_MODELS_DIR

    try:
        import joblib
        import pandas as pd
        import xgboost as xgb

        # ── IsolationForest ───────────────────────────────────────────────
        iso_path = models_dir / "fraud_iso_forest.pkl"
        if not iso_path.exists():
            logger.error("[fraud] fraud_iso_forest.pkl not found at %s", iso_path)
            return False
        _iso_forest = joblib.load(iso_path)
        logger.info("[fraud] IsolationForest loaded (%d estimators)", _iso_forest.n_estimators)

        # ── XGBoost ───────────────────────────────────────────────────────
        xgb_path = models_dir / "fraud_xgboost.json"
        if not xgb_path.exists():
            logger.error("[fraud] fraud_xgboost.json not found at %s", xgb_path)
            return False
        _xgb_model = xgb.XGBClassifier()
        _xgb_model.load_model(str(xgb_path))
        logger.info("[fraud] XGBoost fraud model loaded")

        # ── Feature columns ───────────────────────────────────────────────
        cols_path = models_dir / "fraud_feature_cols.csv"
        if cols_path.exists():
            df = pd.read_csv(cols_path, header=None)
            _feature_cols = (
                df.iloc[1:, 0].tolist()
                if df.iloc[0, 0] == "feature"
                else df.iloc[:, 0].tolist()
            )
            logger.info("[fraud] %d feature columns loaded", len(_feature_cols))

        # ── IsolationForest normalisation params ──────────────────────────
        norm_path = models_dir / "iso_norm_params.csv"
        if norm_path.exists():
            norm_df = pd.read_csv(norm_path, index_col=0)
            _iso_s_min = float(norm_df.loc["s_min", "value"])
            _iso_s_max = float(norm_df.loc["s_max", "value"])
            logger.info("[fraud] ISO norm: s_min=%.4f s_max=%.4f", _iso_s_min, _iso_s_max)

        _loaded = (_iso_forest is not None and _xgb_model is not None)
        return _loaded

    except ImportError as exc:
        logger.error("[fraud] Missing library: %s", exc)
        return False
    except Exception as exc:
        logger.error("[fraud] Load error: %s", exc, exc_info=True)
        return False


def is_available() -> bool:
    return _loaded and _iso_forest is not None and _xgb_model is not None


# ── Feature engineering ───────────────────────────────────────────────────────

def _build_features(claim) -> dict:
    """
    Build the 34-feature dict from a Claim instance.
    All values have explicit fallbacks — no silent NaN.
    """
    from django.utils import timezone
    from datetime import timedelta
    from apps.claims.models import Claim as ClaimModel

    worker  = claim.worker
    event   = claim.disruption_event
    profile = getattr(worker, "workerprofile", None)
    now     = timezone.now()
    filed_at = claim.created_at or now

    # ── Claim velocity ────────────────────────────────────────────────────
    last_30d = ClaimModel.objects.filter(
        worker=worker,
        created_at__gte=now - timedelta(days=30),
    ).exclude(pk=claim.pk).count()

    last_6m = ClaimModel.objects.filter(
        worker=worker,
        created_at__gte=now - timedelta(days=180),
    ).exclude(pk=claim.pk).count()

    # ── Timing ────────────────────────────────────────────────────────────
    claim_hour   = filed_at.hour
    is_odd_hour  = int(claim_hour < 6 or claim_hour > 22)
    is_weekend   = int(filed_at.weekday() >= 5)
    event_start  = event.started_at if event else filed_at
    delay_hours  = max((filed_at - event_start).total_seconds() / 3600, 0)
    instant_flag = int(delay_hours < 0.0833)
    very_fast    = int(delay_hours < 0.5)

    # ── GPS ───────────────────────────────────────────────────────────────
    gps_missing = gps_far = gps_very_far = 0
    gps_distance_km = 0.0

    if profile and event:
        w_zone = profile.zone
        e_zone = event.zone
        if w_zone and e_zone and w_zone.pk != e_zone.pk:
            try:
                gps_distance_km = _haversine(
                    float(w_zone.lat), float(w_zone.lng),
                    float(e_zone.lat), float(e_zone.lng),
                )
            except Exception:
                gps_distance_km = 5.0
            gps_far      = int(gps_distance_km > 5.0)
            gps_very_far = int(gps_distance_km > 15.0)
        elif not w_zone or not e_zone:
            gps_missing = 1
    else:
        gps_missing = 1

    # ── KYC / account ────────────────────────────────────────────────────
    kyc_verified = 0
    try:
        kyc_verified = int(worker.kyc.status == "verified")
    except Exception:
        pass

    tenure_days   = (now.date() - worker.date_joined.date()).days
    tenure_months = max(tenure_days / 30, 0)
    risk_score    = getattr(profile, "risk_score", 0.5) if profile else 0.5
    worker_rating = round(5.0 * (1.0 - risk_score), 2)

    # ── Amount zscore ─────────────────────────────────────────────────────
    MEAN_COV = 1166.0
    STD_COV  = 620.0
    amount   = float(claim.payout_amount)
    amount_zscore = (amount - MEAN_COV) / STD_COV
    high_amount   = int(amount > 1500)

    # ── Zone density ──────────────────────────────────────────────────────
    zone_density = 0
    if event and event.zone:
        zone_density = ClaimModel.objects.filter(
            disruption_event__zone=event.zone,
            created_at__gte=now - timedelta(hours=24),
        ).count()

    velocity_ratio = last_30d / max(zone_density, 1)

    # ── Platform one-hot ──────────────────────────────────────────────────
    platform  = getattr(profile, "platform", "other") if profile else "other"
    seg_food  = int(platform in ("zomato", "swiggy"))
    seg_ecomm = int(platform in ("amazon",))
    seg_groc  = int(platform in ("zepto", "blinkit", "dunzo"))

    # ── Composite signals ─────────────────────────────────────────────────
    network_risk  = int(last_30d > 5 or gps_very_far)
    fraud_signal  = round(
        0.3  * gps_very_far
        + 0.2 * (1 - kyc_verified)
        + 0.2 * int(last_30d > 5)
        + 0.15 * is_odd_hour
        + 0.15 * instant_flag,
        4,
    )
    trust_score = round(max(1.0 - fraud_signal, 0.0), 4)

    # ── Event type encoding ───────────────────────────────────────────────
    EVENT_MAP = {
        "heavy_rain": 0, "extreme_heat": 1, "severe_aqi": 2,
        "flash_flood": 3, "curfew_strike": 4, "platform_down": 5,
    }
    event_type_enc = EVENT_MAP.get(
        event.trigger_type if event else "heavy_rain", 0
    )

    return {
        "claim_amount_inr":                 amount,
        "claim_frequency_last_30_days":     last_30d,
        "claim_frequency_last_6_months":    last_6m,
        "claim_hour":                       claim_hour,
        "event_type_encoded":               event_type_enc,
        "gps_distance_from_event_km":       round(gps_distance_km, 3),
        "gps_missing_flag":                 gps_missing,
        "number_of_devices_used":           1,
        "worker_online_status_during_event": 1,
        "claim_submission_delay_hours":     round(delay_hours, 4),
        "claims_filed_within_1hr":          int(delay_hours < 1.0),
        "same_device_multiple_workers":     0,
        "shared_bank_account_flag":         0,
        "referral_cluster_size":            1,
        "zone_claim_density":               zone_density,
        "tenure_months":                    round(tenure_months, 2),
        "account_age_days":                 tenure_days,
        "worker_rating":                    worker_rating,
        "is_odd_hour":                      is_odd_hour,
        "is_weekend":                       is_weekend,
        "instant_claim_flag":               instant_flag,
        "very_fast_claim":                  very_fast,
        "claim_velocity_ratio":             round(velocity_ratio, 4),
        "gps_far_from_zone":                gps_far,
        "gps_very_far":                     gps_very_far,
        "device_shared_risk":               0,
        "claim_amount_zscore":              round(amount_zscore, 4),
        "high_amount_flag":                 high_amount,
        "network_fraud_risk":               network_risk,
        "fraud_signal_composite":           fraud_signal,
        "trust_score":                      trust_score,
        "delivery_segment_food":            seg_food,
        "delivery_segment_ecommerce":       seg_ecomm,
        "delivery_segment_grocery":         seg_groc,
    }


# ── Public API ────────────────────────────────────────────────────────────────

def score_claim(claim) -> float:
    """
    Run IsolationForest + XGBoost on a claim.
    Returns combined fraud score in [0.0, 1.0].
    Higher = more likely fraud.
    Falls back to 0.0 if models unavailable.
    """
    if not is_available():
        _load()

    if not is_available():
        logger.warning("[fraud] Models unavailable — returning 0.0")
        return 0.0

    try:
        import pandas as pd

        features = _build_features(claim)
        df = pd.DataFrame([features])

        if _feature_cols:
            for col in _feature_cols:
                if col not in df.columns:
                    df[col] = 0
            df = df[_feature_cols]

        # ── IsolationForest score ─────────────────────────────────────────
        iso_raw = _iso_forest.score_samples(df)[0]
        if _iso_s_min is not None and _iso_s_max is not None:
            iso_range = _iso_s_max - _iso_s_min
            if iso_range > 0:
                iso_score = 1.0 - max(0.0, min(1.0,
                    (iso_raw - _iso_s_min) / iso_range
                ))
            else:
                iso_score = float(1 / (1 + math.exp(iso_raw)))
        else:
            iso_score = float(1 / (1 + math.exp(iso_raw)))

        # ── XGBoost score ─────────────────────────────────────────────────
        try:
            xgb_score = float(_xgb_model.predict_proba(df)[0][1])
        except (ValueError, AttributeError):
            xgb_score = float(_xgb_model.predict(df)[0])
            xgb_score = max(0.0, min(1.0, xgb_score))

        # ── Ensemble: 60% XGB + 40% ISO ──────────────────────────────────
        combined = round(0.6 * xgb_score + 0.4 * iso_score, 4)

        logger.debug(
            "[fraud] Claim #%s — iso=%.4f xgb=%.4f combined=%.4f",
            claim.pk, iso_score, xgb_score, combined,
        )
        return combined

    except Exception as exc:
        logger.error("[fraud] Scoring error claim #%s: %s", claim.pk, exc, exc_info=True)
        return 0.0


# ── Geometry helper ───────────────────────────────────────────────────────────

def _haversine(lat1, lon1, lat2, lon2) -> float:
    R = 6371.0
    p1, p2 = math.radians(lat1), math.radians(lat2)
    dp = math.radians(lat2 - lat1)
    dl = math.radians(lon2 - lon1)
    a  = math.sin(dp/2)**2 + math.cos(p1)*math.cos(p2)*math.sin(dl/2)**2
    return R * 2 * math.atan2(math.sqrt(a), math.sqrt(1 - a))